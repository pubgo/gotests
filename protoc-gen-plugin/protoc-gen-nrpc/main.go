...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/pubgo/gotests/protoc-gen-plugin/protoc-gen-nrpc/gen"
...
func main() {
	...
	//fmt.Println("GetFileToGenerate", request.GetFileToGenerate())
	...
	tmpl, err := template.New(".").Funcs(funcMap).Parse(gen.TFile)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, name := range request.GetFileToGenerate() {
		...
		f, err := os.OpenFile("sss.txt", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0755)
		xerror.Exit(err)

		//f.WriteString(fmt.Sprintln("name", name))
		//fmt.Println("FileDescriptorProto", fd.String())
		dt := xerror.PanicBytes(json.MarshalIndent(fd, "", "  "))
		f.Write(dt)
		f.WriteString(fd.String() + "\n")
		for _, s := range fd.Service {
			f.WriteString(s.Method[0].Options.String() + "\n")
			f.WriteString(string(xerror.PanicBytes(json.Marshal(s.Method[0].Options.UninterpretedOption))) + "\n")
		}
