...
func main() {
	...
	interval := flag.Duration("interval", time.Millisecond*500, "When url is provided, defines the interval between fetches."+
		" Note that counter fields are computed based on this interval.")
	steps := flag.Int("steps", 500, "Number of values to plot.")
	rows := flag.Int("rows", 100, "Limits the height of the graph output.")
	flag.Parse()
